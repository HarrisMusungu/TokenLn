...
impl SemanticAnalyzer for CargoTestAnalyzer {
    fn analyze(&self, parsed_failures: &[ParsedFailure]) -> DeviationReport {
        ...
        DeviationReport::new("cargo test", deviations)
    }
}
...
fn to_deviation(failure: &ParsedFailure) -> Deviation {
    ...
    let expected = failure
        ...
        .unwrap_or_else(|| "test should complete without panic".to_string());
    ...
    let actual = match (
        ...
        (None, Some(message)) => format!("panic occurred during test execution: {message}"),
        (None, None) => "panic occurred during test execution".to_string(),
    };
    ...
    let trace = if failure.trace.is_empty() {
        vec!["cargo test".to_string(), test_name.clone()]
    } else {
        failure.trace.clone()
    };
...
fn score_confidence(failure: &ParsedFailure, trace: &[String]) -> (f32, Vec<String>) {
    ...
    if failure.file.is_some() && failure.line.is_some() && failure.column.is_some() {
        score += 0.2;
        reasons.push("precise_location:+0.20".to_string());
    } else if failure.file.is_some() && failure.line.is_some() {
        score += 0.15;
        reasons.push("coarse_location:+0.15".to_string());
    }
    ...
    if failure.assertion_left.is_some() && failure.assertion_right.is_some() {
        ...
        score += 0.15;
        reasons.push("panic_message:+0.15".to_string());
    }
    ...
    if failure
        .panic_message
        .as_ref()
        .is_some_and(|msg| msg.contains("assertion"))
    {
        score += 0.05;
        reasons.push("assertion_text:+0.05".to_string());
    }
