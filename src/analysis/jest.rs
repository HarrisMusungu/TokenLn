...
pub struct JestAnalyzer;

impl SemanticAnalyzer for JestAnalyzer {
    fn analyze(&self, parsed_failures: &[ParsedFailure]) -> DeviationReport {
        ...
        DeviationReport::new("jest", deviations)
    }
}
...
fn to_deviation(failure: &ParsedFailure) -> Deviation {
    ...
    let expected = failure
        ...
        .unwrap_or_else(|| "jest expectation should hold".to_string());
    ...
    let actual = match (
        ...
        (None, Some(message)) => format!("jest reported failure: {message}"),
        (None, None) => "jest reported failure".to_string(),
    };
    ...
    let trace = if failure.trace.is_empty() {
        vec!["jest".to_string(), test_name.clone()]
    } else {
        failure.trace.clone()
    };
...
fn score_confidence(failure: &ParsedFailure, trace: &[String]) -> (f32, Vec<String>) {
    ...
    if failure
        .panic_message
        .as_ref()
        .is_some_and(|msg| msg.contains("expect(received)"))
    {
        score += 0.05;
        reasons.push("expect_marker:+0.05".to_string());
    }
